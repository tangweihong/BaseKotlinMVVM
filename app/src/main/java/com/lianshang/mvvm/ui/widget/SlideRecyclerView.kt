...
class SlideRecyclerView(context: Context, @Nullable attrs: AttributeSet?, defStyle: Int) : RecyclerView(context, attrs, defStyle) {
        ...
        when (e.getAction()) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (mPosition != INVALID_POSITION) {
                    ...
                    // 获取触碰点所在的view
                    mFlingView = getChildAt(mPosition - (getLayoutManager() as LinearLayoutManager).findFirstVisibleItemPosition()) as ViewGroup?
                    LogUtils.d("mFlingView=$mFlingView")
                    ...
                    mMenuViewWidth = if (mFlingView?.getChildCount() === 2) {
                        mFlingView?.getChildAt(1)?.getWidth()?:0
                    } else {
                        INVALID_CHILD_WIDTH
                    }
                }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                val xVelocity: Float = mVelocityTracker?.getXVelocity()?:0F
                val yVelocity: Float = mVelocityTracker?.getYVelocity()?:0F
        ...
        if (mIsSlide && mPosition != INVALID_POSITION) {
            ...
            when (e.getAction()) {
                ...
                MotionEvent.ACTION_UP -> {
                    if (mMenuViewWidth != INVALID_CHILD_WIDTH) {
                        val scrollX: Int = mFlingView!!.getScrollX()
                        mVelocityTracker!!.computeCurrentVelocity(1000)
                        // 此处有两个原因决定是否打开菜单：
                        // 1.菜单被拉出宽度大于菜单宽度一半；
                        // 2.横向滑动速度大于最小滑动速度；
    ...
    fun pointToPosition(x: Int, y: Int): Int {
        val firstPosition: Int = (getLayoutManager() as LinearLayoutManager).findFirstVisibleItemPosition()
